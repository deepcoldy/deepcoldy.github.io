...
|-------|-----------------|
| `dedupKey` | **Nothing on the trigger path consumes it.** It is only rendered into the model prompt along with the rest of the options JSON, and it participates in the idempotency `requestHash`. Real event dedupe lives on the [Webhook connector](/botmux/en/webhook.md)'s dedupe field, not here. |
...
> This is the section the old page was missing most. Read it alongside the [Webhook connector's "Which group does it go to"](/botmux/en/webhook.md).
...
* Message not visible / chat\_id unreadable → 400 `target_required`.
...
`/api/trigger` has **no** "spin up a fresh group per event" capability — `chatId` must be an existing group. That capability exists only as the [Webhook connector](/botmux/en/webhook.md)'s "new group each time" delivery mode (which also pulls the bot's authorized users in automatically and supports dedupe by a dot-path into the event body). Use a connector for that; do not look for it here.
...
Callers go through the dashboard proxy, so `not_found` shows up in two shapes; **normalize both to the not\_found terminal**:
...
Everything above describes the **native four-state contract** (raw dashboard proxy → task-orchestrator callers). The [Webhook connector](/botmux/en/webhook.md)'s own async-poll route goes through a **legacy-consumer adapter**: it rewrites `failed` and `not_found` to `ok:false` and turns HTTP 200 into **404** (the `.state` field is preserved).
...
**[Webhook connector](/botmux/en/webhook.md) only (5)**
...
* Sort polling results into the 5 classes above; never blur "retryable" into "terminal". **not\_found** (404 unknown\_session / `state:not_found`) and **error** (400 request errors, 401/403 auth) are terminal; **unknown** (network error, timeout, 5xx, 502, non-JSON) is retryable — the task may still be running, and compensating as if it were absent causes duplicate execution.
...
For "spin up a fresh group per event", see the [Webhook connector](/botmux/en/webhook.md) — that is its own delivery mode and does not exist on this endpoint.
