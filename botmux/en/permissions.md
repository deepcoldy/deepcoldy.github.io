...
* Team-trust legs (same-team peer bots / team bots / platform team members) grant talk only; see [Roles & Teams](/botmux/en/roles.md).
* Field-level reference: [bots.json · Permissions and authorization](/botmux/en/bots-json.md).
...
| Duty-group instant Q\&A without repo picking | `/oncall bind ~/projects/foo` — see [On-Call Mode](/botmux/en/oncall.md) |
...
* The block list governs **talk access only**; the [message listener](/botmux/en/bots-json.md#group-message-listener) feature (rules that make a bot watch messages automatically) has its own matching configuration and is not affected by it.
...
For more Q\&A see the [FAQ / Troubleshooting](/botmux/en/faq.md); for when @-mentioning is required, see [@ Policy](/botmux/en/mention-mode.md).
