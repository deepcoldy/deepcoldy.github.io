...
> This corresponds to the `defaultOncall` field in `bots.json`, which can also be edited manually. See [bots.json configuration](/botmux/en/bots-json.md).
...
> Want semi-trusted members' **changes to also never touch the real repo**? Enable the [File Sandbox](/botmux/en/sandbox.md) — all writes are isolated and only land after the owner reviews the diff.
