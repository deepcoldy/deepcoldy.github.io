...
Project-group coordinators separately receive Botmux's fixed `<project_group_mode>` protocol for durable project state and the pinned project card. It is not a third Role layer and does not follow the Role's every-turn/once-only injection setting: custom Roles define responsibilities, style, and domain boundaries, while the project protocol governs state lifecycle; both apply together.
...
Role + capability tags are the infrastructure for [multi-bot collaboration](/botmux/en/multi-bot.md): giving each bot a clear identity and responsibilities makes the model less likely to get confused when @-mentioned in the group, with each one playing its part (e.g., one orchestrating, one doing implementation / review).
...
* **Distinct from `/cd`**: the slash command `/cd <path>` (see [Slash Commands](/botmux/en/slash-commands.md)) is the general "change working directory and restart", any directory, owner operate permission; role switch stays inside the role library and is driven by the role protocol — they are not the same thing.
