...
3. **Message-listener match**: once a [message-listening rule](/botmux/en/bots-json.md#group-message-listener) configured under Dashboard **Roles** matches, it fires per the rule — this clause is **independent of both the mention policy and the regular talk gate** (the listener has its own sender filters).
...
> Related: whether a reply lands in a new topic or the current session is controlled by `/reply-mode`; see [Slash Commands](/botmux/en/slash-commands.md).
