...
When a CLI enters a botmux session, it automatically gets `~/.botmux/bin` in its PATH, along with a set of ready-to-use capabilities. This is the channel through which a CLI agent can **proactively** interact with a Lark topic — and "terminal has output but nothing was sent to Lark" usually traces back to the model not using this channel (see [FAQ · C](/botmux/en/faq.md#c-terminal-has-output-but-nothing-sent-to-lark)).
...
| `botmux bots list` | List the bots in the current group and their open\_id (for `--mention`) |
