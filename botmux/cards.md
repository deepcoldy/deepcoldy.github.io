...
* 该能力没有持久重试日志，重启后的恢复也刻意保持很窄。daemon 重启时，Botmux 只会在当前群读取一次现有 Pin 来证明“它确实属于当前 bot 应用”：只有飞书返回 `operator_id_type: "app_id"` 且 `operator_id` 与当前 `larkAppId` 完全一致时，远端 Pin 才算可证明；即便如此，后续清理权限仍然只限于与本进程入队瞬间已知的本地候选 ID 做**严格交集**。Botmux 不会对任意远端 Pin 做宽扫清理；读取或 Pin API 失败时继续 fail-open。显式从 on 切到 off 仍会清理当前会话已知 ID；若来源记录已丢失且无法落入这组同 app 交集，后续关闭或转移会保留这个有歧义的 Pin，而不是冒险清掉人工 Pin。
