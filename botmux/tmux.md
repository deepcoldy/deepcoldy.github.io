...
不会静默降级 pty，而是**硬拦截**：起新会话时若本机 tmux 不可用，botmux 拒绝启动并弹一张卡告诉你装 tmux（`brew install tmux` / `apt-get install -y tmux`），附上 `BACKEND_TYPE=pty` 应急开关。已经活着的常驻会话不受一次性探测失败影响，仍可重连。详见 [前置要求](/botmux/prerequisites.md)。
...
attach 进去后你看到的就是和本地开发完全一致的终端——这也是 botmux 相比"只读输出"方案的关键区别。飞书话题、Web 终端、本地 tmux 三处看到的是同一个进程，见 [Web 终端](/botmux/web-terminal.md)。
...
> ⚠️ pty 会话**不跨 daemon 重启存活**：pty 没有可 re-attach 的常驻进程，重启后会话需重载。要「重启不丢上下文」请用默认的 tmux。也可 per-bot 配 `"backendType": "pty"`（见 [bots.json 配置](/botmux/bots-json.md)）。
