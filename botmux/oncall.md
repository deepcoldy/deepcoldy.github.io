...
> **拉群本身不等于授权**：bot 被拉进群不会自动放开对话权。群里任何人即问即答是 **oncall 绑定生效之后**才有的效果（或用裸 `/grant` 整群放开）；没绑 oncall、也没授权的限制态 bot，非名单成员 @ 它会被拦并触发授权申请卡。详见[权限与授权 · 入群不等于授权](/botmux/permissions.md#入群不等于授权)。
...
> 对应 `bots.json` 字段 `defaultOncall`，也可手动编辑。见 [bots.json 配置](/botmux/bots-json.md)。
...
> 想让半可信成员的**改动也不碰真实仓库**？开 [文件沙盒](/botmux/sandbox.md)——写操作全隔离、owner 审阅 diff 后才落盘。
