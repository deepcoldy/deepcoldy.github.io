...
> 执行行为：任务先按执行位置确定目标。显式设置 `--topic` 时，若目标话题的会话还活着，prompt 直接注入现有会话（不另起 worker）；否则新拉一个 worker，使用任务保存的工作目录。群顶层任务按 Bot / 群的会话模式选择会话。`--new-topic` 每次使用全新会话；配合 `--silent` 时，仅在首次 `botmux send` 需要发送内容时创建话题。
