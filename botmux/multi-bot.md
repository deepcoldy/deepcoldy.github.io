...
现在主路径不再需要手动 `/introduce`：botmux 会默认通过飞书群机器人列表自动发现当前群里的 bot。**跨部署**互相识别有两条路径，都进入**同一道团队信任闸、免 `/grant`**：一是 Dashboard「团队」面板的联邦（把别人的部署拉进同一团队、跨部署拉群）；二是中心平台的 team-sync（平台下发团队与群）。普通 `/group`、Dashboard「新建群」建的是本部署内的普通群，不走这两条、不带跨部署团队信任。
