...
| 字段 | 真实行为 |
|------|---------|
| `dedupKey` | **触发路径没有任何消费方**。它只会随 options JSON 一起渲染进模型提示词，并参与幂等 `requestHash`。真正的事件去重在 [Webhook 接入点](/botmux/webhook.md)的「去重字段」上，不在这里。 |
...
> 这是老文档缺失最多的一节。对照 [Webhook 接入点的「投递到哪个群」](/botmux/webhook.md)阅读。
...
* 该消息不可见 / 取不到 chat\_id → 400 `target_required`。
...
`/api/trigger` **没有**「每次自动拉个新群」的能力——`chatId` 必须是已存在的群。这个能力只存在于 [Webhook 接入点](/botmux/webhook.md)的「每次新建群」投递模式（它还会自动把机器人的授权用户拉进群，并支持按事件 body 的点号路径做去重）。需要自动建群请走接入点，不要走本端点。
...
调用方走 dashboard 代理，`not_found` 会以两种形式出现，**都应归一成 not\_found 终态**：
...
上面描述的是**原生四态契约**（dashboard 裸代理 → 任务编排器调用方）。[Webhook 接入点](/botmux/webhook.md)自己的异步轮询路由走了一层**遗留消费者适配**：它会把 `failed` 与 `not_found` 改写成 `ok:false`，并把 HTTP 200 改成 **404**（`.state` 字段保留）。
...
**仅 [Webhook 接入点](/botmux/webhook.md) 会遇到的（5 个）**
...
* 轮询按 5 类分流，别把「可重试」和「终止」混为一谈：**not\_found**（404 unknown\_session / `state:not_found`）与 **error**（400 请求错误、401/403 鉴权）是终止态；**unknown**（网络异常/超时/5xx/502/非 JSON）是可重试态——原任务可能仍在后台跑，误当查无补偿重派会导致重复执行。
...
想要「每次自动新建一个群来处置事件」，见 [Webhook 接入点](/botmux/webhook.md)——那是它独有的投递模式，本端点没有。
