...
> 与 [Adopt](/botmux/adopt.md) 的区别：Adopt 是把本机 tmux 里的进程接进飞书；Relay 是把一个已经在 botmux 里跑的会话从 A 群搬到 B 群。
...
* picker 模式在\*\*普通群 / 话题群 / 线程内 / 机器人单聊（DM）\*\*都可用：话题群和线程内目标落在对应话题里；单聊按 DM 模式落点——扁平连续模式（p2pMode `chat`）接进整段单聊会话，话题模式由 `/relay` 消息种一个新话题。
