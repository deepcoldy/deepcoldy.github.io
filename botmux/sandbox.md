...
把 AI 编程 CLI 安全地开放给**半可信**用户的隔离机制——常配合 [Oncall 模式](/botmux/oncall.md) 一起用。
...
详见 [bots.json 配置](/botmux/bots-json.md)。
...
文件沙盒 + [Oncall 模式](/botmux/oncall.md) 是绝配：oncall 把机器人开放给一群人随时 @，沙盒保证这群人的改动**不会动到真实仓库**、且**每笔改动 owner 审阅后才落盘**。半可信、多人、随时改的 oncall 场景的标准搭配。
