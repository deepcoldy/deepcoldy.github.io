...
如果你已经在 [role profile](/botmux/roles.md) 里维护了一套可复用的协作人设，建群时加 `--role-profile <profile>`：
...
不想用命令的话，`botmux dashboard` 的 **Groups** 面板也能可视化建群、把指定 bot 拉进群、自动转让群主、@ 提醒，还能解散群 / 让 bot 退群（关联会话自动清理）。新建群弹窗可以选择 Role Profile；已有群行里的「应用配置集」会跳到 **角色配置集** 页面，并把该群预选为 Apply 目标。详见 [Dashboard 管控面](/botmux/dashboard.md)。
