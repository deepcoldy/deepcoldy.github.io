...
* **试用 Webhook 接入点**（Dashboard「接入点(beta)」，详见 [接入点（Webhook）](/botmux/webhook.md)）：让外部系统（监控告警、CI、工单…）通过一个 webhook 触发机器人在群里说话或跑工作流。可配：触发的机器人、触发方式（单轮对话 / 工作流）、投递到哪个群、校验方式（**令牌**放进 URL 一条 curl 就能触发 / **HMAC 签名**更安全）、以及**处理指令**（告诉机器人收到事件该做什么）。
