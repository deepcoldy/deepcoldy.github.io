...
> 按**使用场景**给推荐配置。基础用法见各功能页；排查类问题见 [常见踩坑](/botmux/pitfalls.md) 与 [FAQ](/botmux/faq.md)。
...
* **方式一 · Webhook 主动推送**（Dashboard「接入点(beta)」，详见 [接入点（Webhook）](/botmux/webhook.md)）：让外部系统（监控告警、CI、工单…）通过一个 webhook 触发机器人在群里说话或跑工作流。可配：触发的机器人、触发方式（单轮对话 / 工作流）、投递到哪个群、校验方式（**令牌**放进 URL 一条 curl 就能触发 / **HMAC 签名**更安全）、以及**处理指令**（告诉机器人收到事件该做什么）。
* **方式二 · 监听现有告警群**（详见 [群消息监听](/botmux/bots-json.md#群消息监听)）：如果你的监控/告警系统**已经有自己的飞书机器人在往某个群发告警**，那就不必再配 webhook——把机器人拉进那个群、在 Dashboard「角色 → 消息监听」开启监听，每条告警自动开工排查。用\*\*黑名单模式 + 勾选「机器人」\*\*即可盯住第三方告警机器人（如 Argos）。注意非 @ / 其他机器人发的消息靠约 30s 历史轮询补齐，最长约 30s 延迟。
* **每个报警自动拉群**：可以配成「每来一个报警自动建一个群、把机器人和**值班人一起拉进去**」；填了去重键就把同类报警合并到同一个群，留空则每条报警一个新群——值班同学直接在群里跟进。配合机器人的 `autoStartOnGroupJoin`（见 [`bots.json` · 主动开工](/botmux/bots-json.md#主动开工)），机器人被拉进新群即自动开工、不用手动 @。
* **不同项目对应不同的报警机器人**：给每个项目一个报警机器人，各自配好带项目背景的默认角色 prompt。
* **不同报警机器人配不同的 oncall 目录**：每个报警机器人 `/oncall bind` 到对应项目目录，报警进来直接在那个仓库里排查。
* 也可叠加[定时任务](/botmux/schedule.md)做**主动巡检播报**：`/schedule 每天9:00 检查昨天的报警趋势并总结`，异常才 @ 人。
