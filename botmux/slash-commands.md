...
| `/mention-mode [always\|topic\|never\|ambient\|status]` | 普通群的 @ 策略：什么时候可以不 @ 也回应。查询需对话权、修改需操作权；**仅普通群可设**（私聊/话题群/会话群会被拒绝）。四模式语义与 8 个免 @ 例外见 [@ 策略](/botmux/mention-mode.md) |
...
裸 `/t` 的选仓/固定目录分支见[会话与话题模型](/botmux/session-model.md)。
...
* 免 @ 不等于免权限，且仍有 8 个免 @ 例外（话题内回复、替身、消息监听器等）——完整语义见 [@ 策略](/botmux/mention-mode.md)。
...
分层、额度、授权申请卡与黑名单详见 [权限与授权](/botmux/permissions.md)。注意：**把 bot 拉进群不等于授权**，限制态 bot 仍只接受名单成员。
...
`@botA @botB /t <prompt>`（各自开新话题）· `@botA @botB /introduce`（让本群机器人互相登记 open\_id，协作时可精确 @ 对方）· `botmux bots list`（查看当前群可协作 bot）
