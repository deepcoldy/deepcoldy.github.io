...
CLI 进入 botmux 会话时，自动获得 `~/.botmux/bin` 在 PATH 中，以及一组开箱即用的能力。这是 CLI agent **主动**与飞书话题交互的通道——「终端有输出但没发飞书」的根因往往就是模型没走这个通道（见 [FAQ · C](/botmux/faq.md#c-终端有输出但没发飞书)）。
...
| `botmux bots list` | 查当前群里的机器人及 open\_id（供 `--mention`） |
