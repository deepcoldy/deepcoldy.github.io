...
> 综合自 README 与社区交流群高频问题，持续补充中。更多坑见 [常见踩坑](/botmux/pitfalls.md)。
...
> 想让 agent 帮你自查，见 [常见踩坑 · 排查通用手法](/botmux/pitfalls.md#排查通用手法)（`botmux logs` 找 spawn 命令本地复现 + Web 终端看真实报错）。

### B. 别人不能用 / 弹授权卡

botmux 权限分两层（完整分层见 [权限与授权](/botmux/permissions.md)，速查见下 [权限怎么分](#权限怎么分谁能操作)）：**对话权**（谁能问）和**操作权**（谁能 `/cd` `/restart` 点按钮）。默认只有 `allowedUsers` 名单内的人有对话权，所以别人 @ 会被拒 / 弹授权卡。**把 bot 拉进群本身不会授权任何成员。**
...
* **群 @ 策略**（必须 @ vs 免 @）：默认多人群必须 @；`/mention-mode topic|never|ambient` 可改，语义与 8 个免 @ 例外见 [@ 策略](/botmux/mention-mode.md)。注意「1 个人 + 1 个 bot」的 1v1 群本来就免 @。
* **oncall 场景**（每工单一个新群、全员免 @ 直接问）：见 [Oncall 模式](/botmux/oncall.md)。
...
* **模型网关侧 403**：跟飞书授权无关，多为环境变量 / 网关 token 问题，常见根因是 bash 用户把变量写在 `.bash_profile` 没被 `bash -i` 读到（见 [常见踩坑](/botmux/pitfalls.md)）。
...
速查：`allowedUsers` 给**操作权**（owner 与管理员，能 `/cd` `/restart` `/close` 点按钮、改 `/card` `/cot` `/mention-mode`）；`allowedChatGroups`（裸 `/grant`，整群无额度）、oncall（整群恒放行 + 绑目录）、`chatGrants` / `globalGrants`（按人、默认 3 条 / 1 小时）给**对话权**；`blockedUsers` 是优先于一切放行腿的全局否决。完整分层、额度、有效期、申请卡见 [权限与授权](/botmux/permissions.md)。
...
默认 `always` 模式下，普通群里明确 @ 才回应，避免多 bot / 多人群里乱抢答。群里发 `/mention-mode` 可改：`topic`（自有话题内免 @）、`never`（全群免 @）、`ambient`（免 @ 但点名叫别人时让路）。修改需操作权，仅普通群可设；另有 8 个免 @ 例外（1 人 1 bot 群、消息监听器等），见 [@ 策略](/botmux/mention-mode.md)。
